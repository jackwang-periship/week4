...
print "*****************************"
print "Time Series"
print "pandas has simple, powerful, and ecient functionality for performing resampling operations" 
print "during frequency conversion (for example, converting secondly data into minutely data)."
print "Firse, let's create an array"
print "of 150 dateTime objects at a frequency of 1 second:"
rng = pd.date_range('1/1/2015', periods=150, freq='S')
print rng
print "Now we'll use that to greate a time series, assigning a random integer to each element of the range:"
time_series = pd.Series(np.random.randint(0, 500, len(rng)), index=rng)
print time_series.head()
print "Next, we'll resample the data by binning the one-second raws into minutes"
print "(and summing the associated values):"
print time_series.resample('1Min').sum()
print "We also have support for time zon conversion. For example,"
print "if we assume the original time series was in UTC:"
ts_utc = time_series.tz_localize('UTC')
print ts_utc.head()
print "We can easily convert it to Eastern time:"
print ts_utc.tz_convert('US/Eastern').head()

print ""
print "*****************************"
print "Reading/Writing to files"
print "Writing to a le is straightforward:"
print ts_utc.to_csv('foo.csv')
print "As is reading:"
new_frame = pd.read_csv('foo.csv')
print new_frame.head()
print "We can also read/write to Excel:"
print new_frame.to_excel('foo.xlsx', sheet_name='Sheet1')
print pd.read_excel('foo.xlsx', 'Sheet1', index_col=None, na_values=['NA']).head()
print "But what if the data is a little. . . messy? Something like this:"
broken_df = pd.read_csv('datasets/bikes.csv')
print broken_df[:3]
print "No problem! The readcsv() function has lots of tools to help wrangle this mess. Here we'll"
print "- change the column separator to a ;"
print "- Set the encoding to 'latin1' (the default is 'utf8')"
print "- Parse the dates in the 'Date' column"
print "- Tell it that our dates have the date first instead of the month first"
print "- Set the index to be the 'Date' column"
fixed_df = pd.read_csv('datasets/bikes.csv', sep=';', encoding='latin1',
